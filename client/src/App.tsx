...
const HIGH_DISCOUNT_COUNTRIES = new Set(
  'AF AO BD BF BI BJ BT CD CF CG CI CM DJ EG ER ET FM GH GM GN GW HT KE KG KH KI KM KP LA LB LK LR LS MD MG ML MM MR MW MZ NE NG NI NP PG PK RU RW SB SD SH SL SN SO SR SS ST SY SZ TD TG TJ TL TM TR TV TZ UA UG UZ VE VN VU YE ZM ZW'.split(' ')
);

const LOW_DISCOUNT_COUNTRIES = new Set(
  'AG BB BS EE ES FR HK HR IL IT JP KN KR KW LC LT LV NL NZ PW SI SK TW VC'.split(' ')
);

const getCheckoutTier = (
  selectedCountry: string,
  calculatedTier?: 'LOW' | 'MID' | 'HIGH' | 'NONE'
): 'LOW' | 'MID' | 'HIGH' | 'NONE' => {
  if (calculatedTier) return calculatedTier;
  if (LOW_DISCOUNT_COUNTRIES.has(selectedCountry)) return 'LOW';
  if (HIGH_DISCOUNT_COUNTRIES.has(selectedCountry)) return 'HIGH';
  return 'NONE';
};
 ...
 const handleUpgrade = (
  ...
  const handleImplement = () => {
    if (!user?.isPro) {
      ...
      const activeTier = getCheckoutTier(country, (result as any)?.discountTier);
        ...
        <div className="grid lg:grid-cols-5 gap-12 items-start mb-24">
          <motion.div className="lg:col-span-2 bg-white/70 backdrop-blur-xl border border-white p-8 rounded-[2.5rem] shadow-2xl space-y-8 text-left">
            ...
            <div className="pt-8 border-t border-slate-100 flex flex-col gap-4">
                <div className="p-5 bg-white border border-slate-100 rounded-2xl flex justify-between items-center group cursor-pointer hover:border-blue-600" onClick={() => handleUpgrade('monthly', getCheckoutTier(country, (result as any)?.discountTier), user)}>
                ...
                <div className="p-5 bg-blue-600 text-white rounded-2xl flex justify-between items-center group cursor-pointer active:scale-95 transition-all" onClick={() => handleUpgrade('annual', getCheckoutTier(country, (result as any)?.discountTier), user)}>
                  ...
                  <div className="bg-white/5 p-6 rounded-4xl border border-white/10 relative">
                    ...
                    {!user?.isPro && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/60 rounded-4xl p-6 text-center">
                        <Lock className="text-blue-500 mb-3" size={20}/><button onClick={() => handleUpgrade('monthly', getCheckoutTier(country, (result as any)?.discountTier), user)} className="bg-white text-slate-900 px-6 py-2.5 rounded-full text-[10px] font-black uppercase shadow-2xl">Unlock Pro Features</button>
                      </div>
                    )}
                  </div>
